...
import =imread('DesmosNerveWrap.png');
...
[center, radius] = imfindcircles(nerve_grayscale,[300,800],"ObjectPolarity","bright","Sensitivity",0.99);
x_center = center(1);
y_center = center(2);
im_width = 1:1:size(2);
...
for i = 1:numel(angle)
  e = x_center + lineLength2*cosd(angle(i));
  h = y_center + lineLength2*sind(angle(i));
  x(i)= e;
  y(i) = h;
...
hold on
for k = 1:j
    plot([x_center x(k)], [y_center-lineLength2 y(k)],"Color",[0 1 1]);
  
    %angle_line_m(k) = (y(k)-(y_center-lineLength2))/(x(k)-x_center);
    %angle_line_int(k) = top_y - angle_line_m(k)*top_x;
    %angle_line_y = (angle_line_m(k)*im_width + angle_line_int(k));
    %angle_line_y(angle_line_y<(y_center-lineLength2)) = 0;
...
for t = 1:k  
    ...
    wrap_y_dist(t) = (y_center+lineLength2) - (size(1)-wrap_y(t));
